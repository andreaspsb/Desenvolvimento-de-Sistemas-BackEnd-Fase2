...
export class RabbitmqPagamentoConsumer implements OnModuleInit {
    constructor(
        private readonly registrarPagamentoAssinatura: RegistrarPagamentoAssinaturaUseCase,
    ) { }
    ...
    async onModuleInit() {
        ...
        const queue = await channel.assertQueue('servico-gestao-pagamentos', {
            durable: true,
        });
    ...
    async handleMessage(content: Buffer): Promise<void> {
        ...
        await this.registrarPagamentoAssinatura.execute({
            codAss: payload.codAss,
